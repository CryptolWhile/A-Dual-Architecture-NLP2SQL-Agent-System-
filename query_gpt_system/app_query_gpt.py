...
if not os.path.exists(REGISTRY_PATH):
    st.error(f" Không tìm thấy file Metadata tại {REGISTRY_PATH}. Hãy chạy script `schema_builder.py` trước!")
    st.stop()
    ...
    with st.chat_message("assistant"):
        try:
            ...
            if st.session_state.confirmed_table_names is None:
                st.markdown("###  Xác nhận Danh sách Bảng")
            ...
            # Bước 6: Chạy SQL
            with st.spinner(" Bước 6: Thực thi SQL (Execution)..."):
                # st.write("**Input:**")
                # st.code(sql_query, language="sql")
